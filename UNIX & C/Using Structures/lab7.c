...
FILE * open_out_file (void)
{
    ...
    fprintf(outfile, "\nAlexander Alestra, Lab 7 output. \n");
...
{
        double b[NCOMPETITORS] = {0};
        double avg = 0, win = 0;
        int i, j, k, w;
        //Finding the best jump of each competitor
        for(i = 0; i < NCOMPETITORS; ++i)
        {
                for(j = 0; j < N_TRIES; ++j)
                {
                        if(b[i] < jump_list[i].tries[j])
                        {
                                b[i] = jump_list[i].tries[j];
                        }
                }
                jump_list[i].best_jump = b[i];
        }
        //Finding the average best of all competitors and the best overall jump
        for(k = 0; k < NCOMPETITORS; ++k)
        {
                avg += b[k];
                if(win < b[k])
                {
        ...
        jump_stats->winning_jump = win;
        jump_stats->average_of_best = avg / NCOMPETITORS;
        //Finding the deviation of each competitor
        for(w = 0; w < NCOMPETITORS; ++w)
        {
                jump_list[w].deviation = jump_stats->winning_jump - jump_list[w].best_jump;
        }
        return;
}
