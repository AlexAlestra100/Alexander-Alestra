...
//			System.out.println("Out of Y axis bounds below 0, turning around");
	...
	private Ant(GameWorld gw) {
		super(90, 250, 250, 255, 0, 0, 0, 5000, gw);
	...
	/**
	 * Draws out the objects and gets its location and color
	 */
	public void draw(Graphics g, Point pCmpRelScrn) {
		//update color
		g.setColor(this.getColor());
		
		//draw circle looking thing
		g.fillArc((int) (pCmpRelScrn.getX() + this.getX()), (int) (pCmpRelScrn.getY() + this.getY()), this.getSize(), this.getSize(), 0, 360);
	}
	
	/*
	 * If any collisions happen this is where it will be detected
	 */
	public void handleCollision(GameObject obj) {
		System.out.println("Handling Collision with " + obj.getClass());
		
		GameWorld gw = this.getWorld();
		
		//handle differnt collsions
		if(obj instanceof Spider)
			gw.collisionSpider();
		else if(obj instanceof Flag)
			gw.collisionFlag(((Flag) obj).getSequenceNumber());
